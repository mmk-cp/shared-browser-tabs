...
For custom plain DNS, Xray defaults to IPv4 answers and `ForceIPv4` for the VLESS endpoint and destination resolution. A local A/B check found that the same tunnel/DNS failed HTTPS connections in dual-stack mode but connected over IPv4. This avoids choosing an advertised but unusable IPv6 path; it does not guarantee website/CAPTCHA acceptance. Operators needing dual-stack or IPv6-only destinations can explicitly set `BROWSER_PROXY_IPV4_ONLY=false` and recreate the container. DNS server IPs are not replaced and cookies are not cleared.
